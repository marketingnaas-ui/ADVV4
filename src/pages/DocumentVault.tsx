...
export const DocumentVault = () => {
  ...
  const [viewerOpen, setViewerOpen] = useState<{ advId: string; type: 'PDF' | 'SLIP' | 'OCR' | 'CLR' | 'TAX' | 'RECEIPT' } | null>(null);
    ...
    <div className="space-y-6 pb-12 animate-in fade-in duration-500" style={{ minHeight: '100%' }}>
      ...
      <div className="ph" style={{ marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button className="btn btn-o btn-sm" onClick={() => toast('📦 ส่งออกรายงานคลังเอกสารรวมทั้งหมด (CSV)', 'ok')}>
            📁 Export Vault Index
          </button>
        </div>
      </div>
          ...
          {activeAdv ? (
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justify: 'space-between', padding: '10px 14px', background: '#f8fafc', border: '1.5px solid var(--bdr)', borderRadius: '8px' }}>
                  <div className="fl" style={{ gap: '10px' }}>
                ...
                {activeAdv.pay ? (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justify: 'space-between', padding: '10px 14px', background: '#fffbeb', border: '1.5px dashed #fcd34d', borderRadius: '8px' }}>
                    <div className="fl" style={{ gap: '10px' }}>
                ...
                {activeAdv.clrs && activeAdv.clrs.length > 0 ? (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justify: 'space-between', padding: '10px 14px', background: '#f8fafc', border: '1.5px dashed var(--bdr)', borderRadius: '8px', color: 'var(--ts)', fontSize: '11px' }}>
                  ...
                  {viewerOpen.type === 'PDF' && (
                    <div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '20px', marginBottom: '24px', fontSize: '12px', border: '1px solid var(--bdr)', padding: '16px', borderRadius: '8px', background: '#fafbfc' }}>
                        ...
                        <div style={{ textAlignment: 'right', borderLeft: '1px solid var(--bdr)', paddingLeft: '20px' }}>
                            ...
                            <tr key={idx} style={{ borderBottom: '1.5px solid var(--bdr)' }}>
                              ...
                              <td style={{ padding: '10px', textAlign: 'center' }}>
                                <span className="badge bp" style={{ background: 'var(--p10)', color: 'var(--p)', fontSize: '10px' }}>
                                  {CATS.find(c => c.id === (it.cat || 'C01'))?.name || 'วัสดุ'}
                                </span>
                              </td>
                      ...
                      <div className="footer-signature" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px', marginTop: '48px', fontSize: '11.5px', borderTop: '1px solid var(--bdr)', paddingTop: '24px' }}>
                        ...
                        <div style={{ textAlign: 'center' }}>
                          ...
                          <div style={{ height: '40px', marginTop: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', color: 'var(--ok)', fontSize: '13px' }}>
                            {activeAdv.appBy ? `SIGNED BY ${activeAdv.appBy}` : 'SYSTEM AUTO APPROVED'}
                          </div>
                          <span className="sig-line" style={{ display: 'block', borderTop: '1px dashed #94a3b8', margin: '6px auto', width: '180px' }}></span>
                  ...
                  {viewerOpen.type === 'CLR' && (
                    <div>
                      <div className="header" style={{ borderBottom: '2.5px solid var(--warn)', paddingBottom: '16px', marginBottom: '24px' }}>
                        <div className="flb">
                          <div>
                            <span style={{ fontSize: '24px', fontWeight: 950, color: '#c2410c' }}>CLEARANCE RECONCILIATION VOUCHER</span>
                            <div style={{ fontSize: '11px', color: 'var(--ts)', marginTop: '4px' }}>ใบแจ้งปิดสัญญายืมเงินทดรองจ่ายอิเล็กทรอนิกส์และภาษีหัก ณ ที่จ่ายครบเสร็จ</div>
                          </div>
                          <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '20px', marginBottom: '24px', fontSize: '12px', border: '1px solid var(--bdr)', padding: '16px', borderRadius: '8px', background: '#fafbfc' }}>
                        ...
                        <div style={{ textAlignment: 'right', borderLeft: '1px solid var(--bdr)', paddingLeft: '20px' }}>
                          <div style={{ margin: '4px 0' }}><b>วงเงินยืมทั้งหมด:</b> ฿{fmt(activeAdv.appAmount)}</div>
                          <div style={{ margin: '4px 0' }}><b>ยอดส่งใบเสร็จล้างสุทธิ:</b> ฿{fmt(activeAdv.clrAmount)}</div>
                          <div style={{ margin: '4px 0' }}><b>ยอดผลต่างค้างชำระ:</b> ฿{fmt(Math.max(0, activeAdv.appAmount - activeAdv.clrAmount))}</div>
                          ...
                          {activeAdv.clrs && activeAdv.clrs.length > 0 ? (
                            activeAdv.clrs.map((clr, idx) => (
                              <tr key={idx} style={{ borderBottom: '1px solid var(--bdr)' }}>
                                <td style={{ padding: '8px' }}>{fmtD(clr.date)}</td>
                      ...
                      <div className="footer-signature" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px', marginTop: '48px', fontSize: '11px', borderTop: '1px solid var(--bdr)', paddingTop: '24px' }}>
                        <div style={{ textAlign: 'center' }}>
                          <span style={{ color: 'var(--ts)' }}>Requester (ผู้ส่งเคลียร์)</span>
                        ...
                        <div style={{ textAlign: 'center' }}>
                          <span style={{ color: 'var(--ts)' }}>Approver (ผู้ตรวจสอบสายงาน)</span>
                          <div style={{ height: '35px', marginTop: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', color: 'var(--ok)' }}>
                            {activeAdv.appBy || 'APPROVED'}
                          </div>
                          <span className="sig-line" style={{ display: 'block', borderTop: '1px dashed #94a3b8', margin: '4px auto', width: '130px' }}></span>
                        </div>
                        <div style={{ textAlign: 'center' }}>
