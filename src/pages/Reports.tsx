...
export const Reports = () => {
  ...
  useEffect(() => {
    try {
      ...
      // If empty, pre-populate with realistic mock records of closed/WAITING_CLEARANCE advances to look fully populated
      if (loaded.length === 0) {
        ...
        localStorage.setItem('advposh_review_transactions', JSON.stringify(preseededList));
      } else {
    ...
    <>
      <div className="ph">
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button className="btn btn-o btn-sm" onClick={exportAccountingCSV}>
            <FileSpreadsheet size={14} style={{ marginRight: '4px', display: 'inline' }} /> Export Accounting Excel (CSV)
          </button>
          <button className="btn btn-o btn-sm" onClick={() => toast('Export CSV (Mock)')}>⬇ CSV Summary</button>
          <button className="btn btn-o btn-sm" onClick={() => toast('Export PDF (Mock)')}>⬇ Audit PDF</button>
        </div>
      </div>
      ...
      <div className="card" style={{ background: '#fff', border: '1.5px solid var(--bdr)' }}>
        <div style={{ display: 'flex', justifyContent: 'between', alignItems: 'center', borderBottom: '1.5px solid var(--bdr)', paddingBottom: '10px', marginBottom: '12px' }}>
          ...
          <button className="btn btn-sm" style={{ background: 'var(--p)', color: '#fff', fontWeight: 'bold' }} onClick={exportAccountingCSV}>
