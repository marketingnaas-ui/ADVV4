...
export const Dashboard = () => {
  const { advances, setPage, toast } = useApp();
    ...
    <>
      <div className="ph">
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button className="btn btn-o btn-sm" onClick={() => toast('Export PDF (Mock)')}>⬇ Export</button>
        </div>
      </div>
        ...
        <div className="card">
          ...
          <div className="exp-g">
            {USERS.filter(u => u.role !== 'approver').map(u => {
              ...
              return <div key={u.id} className="exp-c" style={{ background: `rgba(78,149,141,${.15 + int * .75})` }} onClick={() => setPage('datacenter')} title={`${u.name}: ฿${fmt(out)}`}><div className="exp-n">{u.ini}</div><div className="exp-a">{fmtM(out)}</div></div>;
            })}
            {PROJECTS.map(p => {
              ...
              return <div key={p.id} className="exp-c" style={{ background: `rgba(59,130,246,${.15 + int * .75})` }} onClick={() => setPage('datacenter')} title={`${p.name}: ฿${fmt(out)}`}><div className="exp-n">{p.name.substring(0,4)}</div><div className="exp-a">{fmtM(out)}</div></div>;
            })}
          </div>
